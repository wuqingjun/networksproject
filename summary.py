...
NAMES = ["quenet", "perlingiere", "dorland", "arora", "geaccone", "haedicke", "griffith", "dean", "schoolcraft", "crandall"]
...
def main():
    ...
    print "last name, first name, middle init, email addresses, closeness centrality, betweenness centrality"
    for NAME in NAMES:
        if not m.has_key(NAME): continue

        for employee in m[NAME]:
            print "%s, %s, %s, %s, %.4f, %.4f" % (NAME, employee.first_name, employee.middle_init, employee.email_addresses, nx.closeness_centrality(G, u=employee.id), between_cent[employee.id])
